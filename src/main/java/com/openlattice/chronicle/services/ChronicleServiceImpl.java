...
public class ChronicleServiceImpl implements ChronicleService {
    ...
    public final Map<FullQualifiedName, UUID> propertyTypeIds = Maps.newHashMapWithExpectedSize( PT_FQNS.length );
    public final Map<String, UUID>            entitySetIds    = Maps
            .newHashMapWithExpectedSize( ENTITY_SET_NAMES.length );
    ...
    public ChronicleServiceImpl(
        ...
        for ( FullQualifiedName fqn : PT_FQNS )
            propertyTypeIds.put( fqn, edmApi.getPropertyTypeId( fqn.getNamespace(), fqn.getName() ) );
        for ( String entitySetName : ENTITY_SET_NAMES )
            entitySetIds.put( entitySetName, entitySetsApi.getEntitySetId( entitySetName ) );

        dataKey = edmApi.getEntityType( entitySetsApi.getEntitySet( entitySetIds.get( DATA_ENTITY_SET_NAME ) )
                .getEntityTypeId() ).getKey();
    ...
    private UUID reserveDeviceEntityKeyId(
        ...
        return reserveEntityKeyId(
                entitySetIds.get( DEVICES_ENTITY_SET_NAME ),
                ImmutableList.of( propertyTypeIds.get( STRING_ID_FQN ) ),
                data,
                dataIntegrationApi
        );
    }
    ...
    private UUID reserveUsedByEntityKeyId(
        ...
        data.put( propertyTypeIds.get( FULL_NAME_FQN ), ImmutableSet.of( appPackageName ) );
        data.put( propertyTypeIds.get( PERSON_ID_FQN ), Sets.newHashSet( participantId ) );

        return reserveEntityKeyId(
                entitySetIds.get( USED_BY_ENTITY_SET_NAME ),
                ImmutableList.of( propertyTypeIds.get( FULL_NAME_FQN ),
                        propertyTypeIds.get( DATE_TIME_FQN ),
                        propertyTypeIds.get( PERSON_ID_FQN ) ),
                data,
                dataIntegrationApi
        );
    }
    ...
    private UUID reserveRecordedByEntityKeyId(
        ...
        data.put( propertyTypeIds.get( FULL_NAME_FQN ), Sets.newHashSet( appPackageName ) );

        return reserveEntityKeyId(
                entitySetIds.get( RECORDED_BY_ENTITY_SET_NAME ),
                ImmutableList.of( propertyTypeIds.get( DATE_LOGGED_FQN ),
                        propertyTypeIds.get( STRING_ID_FQN ),
                        propertyTypeIds.get( FULL_NAME_FQN ) ),
                data,
                dataIntegrationApi
        );
    }
    ...
    private UUID reserveUserAppEntityKeyId(
        ...
        return reserveEntityKeyId(
                entitySetIds.get( CHRONICLE_USER_APPS ),
                ImmutableList.of( propertyTypeIds.get( FULL_NAME_FQN ) ),
                entityData,
                dataIntegrationApi
        );
    ...
    private void createUserAppsEntitiesAndAssociations(
        ...
        for ( SetMultimap<UUID, Object> appEntity : data ) {
            try {
                ...
                String appPackageName = appEntity.get( propertyTypeIds.get( FULL_NAME_FQN ) ).iterator().next()
                        .toString();
                String appName = userAppsDict.get( appPackageName );
                if ( appName == null )
                    continue;
                String dateLogged = getMidnightDateTime( appEntity.get( propertyTypeIds.get( DATE_LOGGED_FQN ) )
                        .iterator().next()
                        .toString() );

                // create entity in chronicle_user_apps
                Map<UUID, Set<Object>> userAppEntityData = new HashMap<>();
                userAppEntityData.put( propertyTypeIds.get( FULL_NAME_FQN ), Sets.newHashSet( appPackageName ) );
                userAppEntityData.put( propertyTypeIds.get( TITLE_FQN ), Sets.newHashSet( appName ) );

                UUID userAppEntityKeyId = reserveUserAppEntityKeyId( userAppEntityData, dataIntegrationApi );
                dataApi.updateEntitiesInEntitySet( entitySetIds.get( CHRONICLE_USER_APPS ),
                ...
                recordedByEntityData.put( propertyTypeIds.get( DATE_LOGGED_FQN ), ImmutableSet.of( dateLogged ) );
                recordedByEntityData.put( propertyTypeIds.get( STRING_ID_FQN ), ImmutableSet.of( deviceId ) );

                UUID recordedByEntityKeyId = reserveRecordedByEntityKeyId( recordedByEntityData,
                        appPackageName,
                        dataIntegrationApi );
                dataApi.updateEntitiesInEntitySet( entitySetIds.get( RECORDED_BY_ENTITY_SET_NAME ),
                        ImmutableMap.of( recordedByEntityKeyId, recordedByEntityData ),
                        UpdateType.Merge );

                EntityDataKey src = new EntityDataKey( entitySetIds.get( CHRONICLE_USER_APPS ), userAppEntityKeyId );
                EntityDataKey dst = new EntityDataKey( entitySetIds.get( DEVICES_ENTITY_SET_NAME ), deviceEntityKeyId );
                EntityDataKey edge = new EntityDataKey( entitySetIds.get( RECORDED_BY_ENTITY_SET_NAME ),
                ...
                usedByEntityData.put( propertyTypeIds.get( DATE_TIME_FQN ), ImmutableSet.of( dateLogged ) );

                UUID usedByEntityKeyId = reserveUsedByEntityKeyId( usedByEntityData,
                        appPackageName,
                        participantId,
                        dataIntegrationApi );
                dataApi.updateEntitiesInEntitySet( entitySetIds.get( USED_BY_ENTITY_SET_NAME ),
                        ImmutableMap.of( usedByEntityKeyId, usedByEntityData ),
                        UpdateType.Merge );

                dst = new EntityDataKey( participantEntitySetId, participantEntityKeyId );
                edge = new EntityDataKey( entitySetIds.get( USED_BY_ENTITY_SET_NAME ), usedByEntityKeyId );
                dataEdgeKeys.add( new DataEdgeKey( src, dst, edge ) );
    ...
    private void createAppDataEntitiesAndAssociations(
        ...
        for ( int i = 0; i < data.size(); i++ ) {
            entities.put( entitySetIds.get( DATA_ENTITY_SET_NAME ), Multimaps.asMap( data.get( i ) ) );

            Map<UUID, Set<Object>> recordedByEntity = ImmutableMap
                    .of( propertyTypeIds.get( DATE_LOGGED_FQN ), Sets.newHashSet( timeStamp ) );

            associations.put( entitySetIds.get( RECORDED_BY_ENTITY_SET_NAME ), new DataAssociation(
                    entitySetIds.get( DATA_ENTITY_SET_NAME ),
    ...
    public List<ChronicleAppsUsageDetails> getParticipantAppsUsageData(
        ...
        Map<UUID, List<NeighborEntityDetails>> participantNeighbors = searchApi.executeFilteredEntityNeighborSearch(
                ...
                new EntityNeighborsFilter(
                        ...
                        java.util.Optional.of( ImmutableSet.of( entitySetIds.get( CHRONICLE_USER_APPS ) ) ),
                        java.util.Optional.of( ImmutableSet.of( participantEntitySetId ) ),
                        java.util.Optional.of( ImmutableSet.of( entitySetIds.get( USED_BY_ENTITY_SET_NAME ) ) )
                )
        );
    ...
    public UUID registerDatasource(
        ...
        deviceData.put( propertyTypeIds.get( STRING_ID_FQN ), Sets.newHashSet( datasourceId ) );
        ...
        if ( datasource.isPresent() && AndroidDevice.class.isAssignableFrom( datasource.get().getClass() ) ) {
            ...
            deviceData.put( propertyTypeIds.get( MODEL_FQN ), Sets.newHashSet( device.getModel() ) );
            deviceData.put( propertyTypeIds.get( VERSION_FQN ), Sets.newHashSet( device.getOsVersion() ) );
        }
        ...
        dataApi.updateEntitiesInEntitySet( entitySetIds.get( DEVICES_ENTITY_SET_NAME ),
                ImmutableMap.of( deviceEntityKeyId, deviceData ),
                UpdateType.Merge );

        EntityDataKey deviceEDK = new EntityDataKey( entitySetIds.get( DEVICES_ENTITY_SET_NAME ), deviceEntityKeyId );
        ...
        EntityDataKey studyEDK = new EntityDataKey( entitySetIds.get( STUDY_ENTITY_SET_NAME ), studyEntityKeyId );

        ListMultimap<UUID, DataEdge> associations = ArrayListMultimap.create();

        Map<UUID, Set<Object>> usedByEntity = ImmutableMap
                .of( propertyTypeIds.get( STRING_ID_FQN ), Sets.newHashSet( UUID.randomUUID() ) );

        associations.put( entitySetIds.get( USED_BY_ENTITY_SET_NAME ),
                new DataEdge( deviceEDK, participantEDK, usedByEntity ) );
        associations
                .put( entitySetIds.get( USED_BY_ENTITY_SET_NAME ), new DataEdge( deviceEDK, studyEDK, usedByEntity ) );
    ...
    private Set<UUID> getNotificationEnabledStudies(
        ...
        Map<UUID, List<NeighborEntityDetails>> studyNeighbors = searchApi
                .executeFilteredEntityNeighborSearch(
                        entitySetIds.get( STUDY_ENTITY_SET_NAME ),
                        new EntityNeighborsFilter(
                                studyEntityKeyIds,
                                java.util.Optional
                                        .of( ImmutableSet.of( entitySetIds.get( NOTIFICATION_ENTITY_SET_NAME ) ) ),
                                java.util.Optional.of( ImmutableSet.of( entitySetIds.get( STUDY_ENTITY_SET_NAME ) ) ),
                                java.util.Optional.of( ImmutableSet.of( entitySetIds.get( PART_OF_ENTITY_SET_NAME ) ) )
                        )
                );
    ...
    public void refreshUserAppsDictionary() {
        ...
        Iterable<SetMultimap<FullQualifiedName, Object>> entitySetData = dataApi.loadEntitySetData(
                entitySetIds.get( DICTIONARY_ENTITY_SET_NAME ),
                FileType.json,
                jwtToken
        );
        logger.info(
    ...
    public void refreshStudyInformation() {
        ...
        List<Map<FullQualifiedName, Set<Object>>> studySearchResult = searchApi
                .executeEntitySetDataQuery( entitySetIds.get( STUDY_ENTITY_SET_NAME ),
        ...
        Map<UUID, List<NeighborEntityDetails>> studyNeighbors = searchApi.executeFilteredEntityNeighborSearch(
                ...
                new EntityNeighborsFilter(
                        ...
                        java.util.Optional.of( ImmutableSet.of( entitySetIds.get( PARTICIPATED_IN_AESN ) ) )
                )
        );
                ...
                .putAll( searchApi
                        .executeFilteredEntityNeighborSearch( entry.getKey(),
                                new EntityNeighborsFilter( Sets.newHashSet( entry.getValue() ),
                                        java.util.Optional
                                                .of( ImmutableSet.of( entitySetIds.get( DEVICES_ENTITY_SET_NAME ) ) ),
                                        java.util.Optional.of( ImmutableSet.of() ),
                                        java.util.Optional.empty() ) ) ) );
    ...
    public ChronicleQuestionnaire getQuestionnaire( UUID studyId, UUID questionnaireEKID ) {
        try {
            ...
            Map<UUID, List<NeighborEntityDetails>> neighbors = searchApi.executeFilteredEntityNeighborSearch(
                    ...
                    new EntityNeighborsFilter(
                            Set.of( studyEKID ),
                            java.util.Optional.of( Set.of( entitySetIds.get( QUESTIONNAIRE_ENTITY_SET_NAME ) ) ),
                            java.util.Optional.of( Set.of( entitySetIds.get( STUDY_ENTITY_SET_NAME ) ) ),
                            java.util.Optional.of( Set.of( entitySetIds.get( PART_OF_ENTITY_SET_NAME ) ) )
                    )
            );
            ...
            if ( neighbors.containsKey( studyEKID ) ) {
                ...
                neighbors = searchApi.executeFilteredEntityNeighborSearch(
                        entitySetIds.get( QUESTIONNAIRE_ENTITY_SET_NAME ),
                        new EntityNeighborsFilter(
                                Set.of( questionnaireEKID ),
                                java.util.Optional.of( Set.of( entitySetIds.get( QUESTIONS_ENTITY_SET_NAME ) ) ),
                                java.util.Optional.of( Set.of( entitySetIds.get( QUESTIONNAIRE_ENTITY_SET_NAME ) ) ),
                                java.util.Optional.of( Set.of( entitySetIds.get( PART_OF_ENTITY_SET_NAME ) ) )
                        )
                );
            ...
            Map<UUID, List<NeighborEntityDetails>> neighbors = searchApi
                    .executeFilteredEntityNeighborSearch(
                            ...
                            new EntityNeighborsFilter(
                                    ...
                                    java.util.Optional
                                            .of( Set.of( entitySetIds.get( QUESTIONNAIRE_ENTITY_SET_NAME ) ) ),
                                    java.util.Optional.of( Set.of( entitySetIds.get( STUDY_ENTITY_SET_NAME ) ) ),
                                    java.util.Optional.of( Set.of( entitySetIds.get( PART_OF_ENTITY_SET_NAME ) ) )
                            )
                    );
        ...
        try {
            ...
            for ( int i = 0; i < questionEntityKeyIds.size(); i++ ) {
                ...
                Map<UUID, Set<Object>> answerEntity = ImmutableMap.of(
                        propertyTypeIds.get( VALUES_FQN ),
                        questionnaireResponses.get( questionEntityKeyId ).get( VALUES_FQN ) );
                entities.put( entitySetIds.get( ANSWERS_ENTITY_SET_NAME ), answerEntity );
                ...
                associations.put( entitySetIds.get( RESPONDS_WITH_ENTITY_SET_NAME ), new DataAssociation(
                        participantESID,
                        java.util.Optional.empty(),
                        java.util.Optional.of( participantEKID ),
                        entitySetIds.get( ANSWERS_ENTITY_SET_NAME ),
                        java.util.Optional.of( i ),
                        java.util.Optional.empty(),
                        respondsWithEntity
                ) );
                ...
                Map<UUID, Set<Object>> addressesEntity = ImmutableMap.of(
                        propertyTypeIds.get( COMPLETED_DATE_TIME_FQN ),
                        ImmutableSet.of( dateTime )
                );
                associations.put( entitySetIds.get( ADDRESSES_ENTITY_SET_NAME ), new DataAssociation(
                        entitySetIds.get( ANSWERS_ENTITY_SET_NAME ),
                        java.util.Optional.of( i ),
                        java.util.Optional.empty(),
                        entitySetIds.get( QUESTIONS_ENTITY_SET_NAME ),
                        java.util.Optional.empty(),
